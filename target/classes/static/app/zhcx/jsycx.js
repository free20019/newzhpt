								...
								$('<a>').addClass('btn btn-primary btn-xs').text('详情').css(style).on('click', function () {
									...
									for(var i = 0; i< re.length ;i++){
			         					if(re[i].id_number == item.sfzh){
			         						...
			         						$('#jsycx-dialog-cphm div').html("").html(s.plate_number.replace(".",""));
			         						$('#jsycx-dialog-zgzh div').html("").html(s.ID);
			         						$('#jsycx-dialog-ajzt div').html("").html(formatYYYYMMDD(s.valid_period_start));
			         						$('#jsycx-dialog-yxqz div').html("").html(formatYYYYMMDD(s.valid_period_end));
