...
- Added `popper-app-to-body` attribute for Autocomplete, #12241
...
- Corregida la posición incorrecta del slot Input `suffix`  cuando se usaba el slot `suffix` con el slot  `app`  , #11951
...
- Agregado el atributo `popper-app-to-body` para SubMenu, #10515
...
- Tree
  ...
  - Agregados los metodos  `getNode`, `remove`, `app`, `insertBefore`, `insertAfter`, `getCheckedKeys`, `getHalfCheckedNodes`, `getHalfCheckedKeys` y el evento `check`, #9718 #9730
- Transfer
  - Agregado el metodo `clearQuery`, #9753
- Select
  - Agregado el atributo `popper-app-to-body`, #9782
...
- Corregido el problema de color del borde del Select cuando está el slot `prepend` o `app`  del Input, #9089
...
- Dialog
  - Añadidos los atributos `width`, `fullscreen`, `app-to-body`. Ahora Dialog puede ser anidado
...
- Table
  ...
  - Arreglado un bug que corregía las columnas de una tabla con slot con nombre  `app`   que  desaparecia cuando los datos eran recuperados dinámicamente.
...
- Table
  ...
  - El slot `app`  se desplazo fuera del elemento `tbody` para evitar múltiples renderizados.
