...
- Fixed wrong position of Input `suffix` slot when `suffix` slot with `app` slot, #11951
...
- Added `popper-app-to-body` attribute for SubMenu, #10515
...
- Tree
  ...
  - Added `getNode`, `remove`, `app`, `insertBefore`, `insertAfter`, `getCheckedKeys`, `getHalfCheckedNodes`, `getHalfCheckedKeys` methods and `check` event, #9718 #9730
- Transfer
  - Added `clearQuery` method, #9753
- Select
  - Added `popper-app-to-body` attribute, #9782
...
- Fixed border color issue of Select when in `prepend` or `app` slot of Input, #9089
...
- Dialog
  - Added `width`、`fullscreen`、`app-to-body` attributes. Now Dialog can be nested
...
- Table
  ...
  - Fixed a bug that fixed columns of a Table with `app` slot would disappear when data is dynamically fetched
...
- Table
  ...
  - `app` slot is moved outside the `tbody` element to avoid multiple rendering
