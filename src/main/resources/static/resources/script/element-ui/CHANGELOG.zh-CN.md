...
- 添加 Autocomplete 的 popper-app-to-body 属性，#12241
...
- 修复 Input 组件 `suffix` 与 `app` 共存时样式错乱问题，#11951
...
- 新增 SubMenu 的 `popper-app-to-body` 属性，#10515
...
- Tree
  ...
  - 新增 `getNode`、`remove`、`app`、`insertBefore`、`insertAfter`、`getCheckedKeys`、`getHalfCheckedNodes`、`getHalfCheckedKeys` 方法和 `check` 事件，#9718 #9730
- Transfer
  - 新增 `clearQuery` 方法，#9753
- Select
  - 新增 `popper-app-to-body` 属性，#9782
...
- 修复 Input 的 `prepend` 或 `app` slot 中 Select 的边框颜色错误，#9089
...
- Dialog
  - 新增 `width`、`fullscreen`、`app-to-body` 属性，支持嵌套使用
...
- Table
  ...
  - 修复 `app` 具名 slot 和固定列并存时，动态获取表格数据会导致固定列消失的问题
...
- Table
  ...
  - 将 `app` slot 移至 `tbody` 元素以外，以保证其只被渲染一次
