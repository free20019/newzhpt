...
//		$('#clydqyj-datetimeStart').datetimepicker(dateDefaultOption);
//		$('#clydqyj-datetimeEnd').datetimepicker(dateDefaultOption);
		$('#clydqyj-datetimeStart').val(getMyDate(parseInt(new Date().getTime()-6*24*3600*1000)));
		$('#clydqyj-datetimeEnd').val(new Date().Format('yyyy-MM-dd'));
		 $("#clydqyj-datetimeStart").datetimepicker({　
        　　			format: 'yyyy-mm-dd',
	    　　　　　　language: 'zh-CN',
	    　　　　　　weekStart: 1,
//	    　　　　　　todayBtn: 1,//显示‘今日’按钮
	    ...
	    　　　　　　forceParse: 1,
	         ...
	         var starttime=$("#clydqyj-datetimeStart").val();
	         $("#clydqyj-datetimeEnd").datetimepicker('setStartDate',starttime);
	         $("#clydqyj-datetimeStart").datetimepicker('hide');
	    });
	    $("#clydqyj-datetimeEnd").datetimepicker({　
	      　　format: 'yyyy-mm-dd',
	  　　　　　　language: 'zh-CN',
	  　　　　　　weekStart: 1,
//	  　　　　　　todayBtn: 1,//显示‘今日’按钮
	  ...
	  　　　　　　forceParse: 0,
	       ...
	       var endtime=$("#clydqyj-datetimeEnd").val();
	       $("#clydqyj-datetimeStart").datetimepicker('setEndDate',endtime);
	       $("#clydqyj-datetimeEnd").datetimepicker('hide'); 
	  });
		...
		clydqyjmap = new AMap.Map('clydqyjMap', {
			center : position,
			level : 15,
			resizeEnable : true
		});
		clydqyjmap.plugin( [ "AMap.ToolBar", "AMap.OverView", "AMap.Scale" ],
				function() {
					//加载工具条
				...
				clydqyjmap.addControl(tool);
				//加载鹰眼
				view = new AMap.OverView();
				clydqyjmap.addControl(view);
				//加载比例尺
				scale = new AMap.Scale();
				clydqyjmap.addControl(scale);
				clydqyjmap.plugin( [ "AMap.MapType" ], function() {
					var type = new AMap.MapType( {
						defaultType : 0
					});//初始状态使用2D地图
					clydqyjmap.addControl(type);
					});
			});
		
		//查询所有的车辆
		$('#bjcx').on('click',function(){
			bjcx();
		})
		function bjcx(){
			markers=[];
			jqxhr=$.ajax({
	 	        ...
	 	        url:"../../sbwx/findydqbj",
	 	        data:{
	 	        	"stime" : $('#clydqyj-datetimeStart').val(),
	 	        	"etime" : $('#clydqyj-datetimeEnd').val()
	 	        },
	 	        dataType: 'json',
	 			timeout : 3600000,
	 				...
	 				$("#cphm_value").on("keyup",function(){
	 					...
	 					if(this.value.length >= 3){
	 						...
	 						for(var i=0;i<=clydqvehilist.length;i++){
	 							if(clydqvehilist[i] != undefined && clydqvehilist[i].VEHICLE_NO.indexOf(vehi) != -1){
	 								text +='<li class="listItem">'+
	 											'<span class="item-col" type="vehicleNum">'+clydqvehilist[i].VEHICLE_NO+'</span>'+
	 											'<span class="item-col gz" type="button">跟踪</span>'+
//	 											'<span class="col btn btn-primary btn-xs gz" type="button">跟踪</span>'+
	 										'</li>';
	 							}
	 						}
	 						$('#vehi_list').html(text);
		...
		function addmks(obj){
			if(obj.hasOwnProperty('PX')&&obj.hasOwnProperty('PY')){		
				...
				marker1.setMap(clydqyjmap);  //在地图上添加点			
		...
		function addMarker(vehicle){
			...
			clydqyjmap.setCenter(new AMap.LngLat(vehicle.PX,vehicle.PY));
...
//			"<tr><td><b>[维修完成时间]</b>："+(vehicle.RR_TIME_END==null?"":(new Date(vehicle.RR_TIME_END)).Format("yyyy-MM-dd hh:mm:ss"))+"</td></tr>" +
			...
			var inforWindowone = new AMap.InfoWindow({                 
			  ...
			  inforWindowone.open(clydqyjmap,marker.getPosition());                 
		...
		function addCluster(tag){
			...
			if(tag==1) {
				...
				clydqyjmap.plugin(["AMap.MarkerClusterer"],function(){
					cluster = new AMap.MarkerClusterer(clydqyjmap,markers,{minClusterSize:10,styles:sts});
				});
			}else {
				clydqyjmap.plugin(["AMap.MarkerClusterer"],function(){
					cluster = new AMap.MarkerClusterer(clydqyjmap,markers);
				});
			}
		}
		//比较查询出的时间和当前时间
