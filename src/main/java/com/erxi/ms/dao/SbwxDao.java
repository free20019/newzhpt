...
//			sql +=" limit "+((pageIndex-1)*pageSize)+","+pageSize+") tt ";
			...
			String sql = "select (select count(1) COUNT from (select a.* from (select co.COMPANY_NAME COMP_NAME,r.*,v.SIM_NUM,v.COMP_ID,v.OWN_NAME,v.OWN_TEL from tb_vehicle v,tb_global_vehicle co,"
					+ " TB_REPAIR_RECORD r"
			...
			sql += ") m) as COUNT,tt.* from (select al.*,a.*"
					+ " from (select co.COMPANY_NAME COMP_NAME,r.*,v.COMP_ID,v.SIM_NUM,v.OWN_NAME,v.OWN_TEL from tb_vehicle v,tb_global_vehicle co,"
					+ " TB_REPAIR_RECORD r"
...
//					tj += " and a.RR_TIME_END<a.db_time";	
			...
			sql += tj2;
			sql += " ) r, tb_taxi_gzfx al,tb_global_vehicle co"
					+ " where v.vehi_no=r.vehi_no and r.vehi_no=al.vehicle_no and v.vehi_no=co.plate_number";
			sql += tj;				
			sql += ") as COUNT, a.*,r.*"
					+ " from (select  co.COMPANY_NAME COMP_NAME,al.*,v.SIM_NUM,v.COMP_ID,v.OWN_NAME,v.OWN_TEL from tb_vehicle v ,tb_global_vehicle co,"
					+ " (select distinct VEHI_NO from TB_REPAIR_RECORD";
					sql += tj2;
...
//					tj += " and a.RR_TIME_END<a.db_time";	
			...
			String sql = "select a.*,r.* from(select co.COMPANY_NAME COMP_NAME,al.*,r.*,v.SIM_NUM,v.COMP_ID,v.OWN_NAME,v.OWN_TEL from tb_vehicle v,tb_global_vehicle co,"
					+ " (select distinct vehi_no from TB_REPAIR_RECORD";
			sql += tj2;
			sql += " ) r, tb_taxi_gzfx al"
					+ " where v.vehi_no=r.vehi_no and r.vehi_no=al.vehicle_no and v.vehi_no=co.plate_number";
